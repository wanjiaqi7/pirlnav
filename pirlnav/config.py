# 定义了用于配置导航任务和策略训练的配置文件
# 任务配置
# **任务配置**：包括导航任务的各种参数设置，如最大步数、环境参数、相机参数等。
# **奖励设置**：包括了奖励函数的配置，定义了简单奖励和稀疏奖励的类型、成功奖励等参数。
# **数据集配置**：指定了数据集类型和最大的情节步数。
# 训练配置
# **基本训练配置**：指定了训练相关的一般设置，如日志目录、评估频率、检查点目录等。
# **IL 配置**：包括了模仿学习相关的参数设置，如模型名称、优化器参数、批次大小等。
# **策略配置**：定义了策略模型的各种参数，如 RGB 编码器、状态编码器、Seq2Seq 模型、评论家等。
# **默认 RL 配置：定义了强化学习相关的默认参数，如 PPO 相关参数、隐藏层大小等。
# **策略微调配置**：包括了策略微调相关的参数设置，如微调学习率、微调开始步数等。
import warnings
